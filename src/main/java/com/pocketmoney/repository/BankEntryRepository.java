...
public interface BankEntryRepository extends JpaRepository<BankEntry, Long> {
    ...
    List<BankEntry> findByUserNameAndEntryDateBetweenOrderByEntryDateDescCreatedAtDesc(
            String userName, LocalDate startDate, LocalDate endDate);

    List<BankEntry> findByUserNameOrderByEntryDateDescCreatedAtDesc(String userName);

    @Query("SELECT COALESCE(SUM(e.amount), 0) FROM BankEntry e WHERE e.userName = :userName AND e.type = :type")
